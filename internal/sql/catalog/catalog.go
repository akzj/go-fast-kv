...
// tableKey returns the KV key for storing a table schema.
func tableKey(tableName string) []byte {
	return []byte("_sql:table:" + tableName)
}

// indexKey returns the KV key for storing an index schema.
func indexKey(tableName, indexName string) []byte {
	return []byte("_sql:index:" + tableName + ":" + indexName)
}

// tableIndexPrefix returns the KV key prefix for all indexes of a table.
...
func (c *Catalog) DropTable(name string) error {
	...
	// Delete all indexes of this table
...
// GetIndex retrieves an index schema.
...
func (c *Catalog) ListTables() ([]string, error) {
	...
	for iter.Next() {
		key := string(iter.Key())
		// Extract table name from "_sql:table:{name}"
		if len(key) > 11 {
...
// ListIndexesByTable lists all indexes for a table.
