...
func TestExec_UniqueConstraint_Debug(t *testing.T) {
	env := newTestEnv(t)
	...
	// Check indexes
	indexes, _ := env.cat.ListIndexes("users")
	t.Logf("Indexes after CREATE TABLE: %d", len(indexes))
	for _, idx := range indexes {
		t.Logf("  Index: %s, Unique: %v", idx.Name, idx.Unique)
	}
	...
	if err != nil {
		...
		t.Log("No error (bad) - duplicate was inserted!")
	}
}

func TestExec_NotNullConstraint(t *testing.T) {
	env := newTestEnv(t)
	
	// Create table with NOT NULL column
	env.execSQL(t, "CREATE TABLE users (id INT PRIMARY KEY, name TEXT NOT NULL, email TEXT)")
	
	// Verify NOT NULL flag is stored in catalog
	tbl, err := env.cat.GetTable("users")
	if err != nil {
		t.Fatalf("GetTable: %v", err)
	}
	...
	if !tbl.Columns[1].NotNull {
		t.Errorf("name column should have NotNull=true")
	}
	if tbl.Columns[0].NotNull {
		t.Errorf("id column should have NotNull=false")
	}
	if tbl.Columns[2].NotNull {
		t.Errorf("email column should have NotNull=false")
	}
	
	// Test: INSERT with NULL into NOT NULL column should fail
	t.Run("insert_null_into_not_null", func(t *testing.T) {
		result, err := env.execSQLErr(t, "INSERT INTO users VALUES (1, NULL, 'test@example.com')")
		if err == nil {
			t.Errorf("expected error for NULL into NOT NULL column, got result: %+v", result)
		}
		if result != nil {
	...
	// Test: INSERT with valid values should succeed
	t.Run("insert_valid", func(t *testing.T) {
		env.execSQL(t, "INSERT INTO users VALUES (1, 'Alice', 'alice@example.com')")
		
		result := env.execSQL(t, "SELECT id, name FROM users WHERE id = 1")
		if len(result.Rows) != 1 {
			t.Fatalf("expected 1 row, got %d", len(result.Rows))
		}
		if result.Rows[0][1].Text != "Alice" {
	...
	// Test: UPDATE with NULL into NOT NULL column should fail
	t.Run("update_null_into_not_null", func(t *testing.T) {
		// First insert a valid row
		env.execSQL(t, "INSERT INTO users VALUES (2, 'Bob', 'bob@example.com')")
		
		// Update name to NULL - should fail
		result, err := env.execSQLErr(t, "UPDATE users SET name = NULL WHERE id = 2")
		if err == nil {
			t.Errorf("expected error for NULL into NOT NULL column, got result: %+v", result)
		}
	})
	
	// Test: UPDATE with valid value should succeed
	t.Run("update_valid", func(t *testing.T) {
		env.execSQL(t, "UPDATE users SET name = 'Robert' WHERE id = 2")
		
		result := env.execSQL(t, "SELECT name FROM users WHERE id = 2")
		if len(result.Rows) != 1 {
			t.Fatalf("expected 1 row, got %d", len(result.Rows))
		}
		if result.Rows[0][0].Text != "Robert" {
			t.Errorf("expected name 'Robert', got %s", result.Rows[0][0].Text)
		}
	})
}

func TestDebug_PlannerUniqueIndexes(t *testing.T) {
	env := newTestEnv(t)
	
	stmt, err := env.parser.Parse("CREATE TABLE users (id INT PRIMARY KEY, email TEXT UNIQUE)")
	if err != nil {
	...
	ctPlan, ok := plan.(*plannerapi.CreateTablePlan)
	if !ok {
		t.Fatalf("expected CreateTablePlan, got %T", plan)
	}
	
	t.Logf("UniqueIndexes count: %d", len(ctPlan.UniqueIndexes))
	for _, idx := range ctPlan.UniqueIndexes {
		t.Logf("  %s.%s unique=%v", idx.Table, idx.Column, idx.Unique)
	}
	
	if len(ctPlan.UniqueIndexes) == 0 {
		t.Error("UniqueIndexes is empty - planner not populating it")
	}
}

package internal

func TestDebug_VerifyCall(t *testing.T) {
	env := newTestEnv(t)
	...
	// Try to manually call checkUniqueConstraint via exec
	_, err := env.execSQLErr(t, "INSERT INTO users VALUES (2, 'alice@example.com')")
	if err != nil {
